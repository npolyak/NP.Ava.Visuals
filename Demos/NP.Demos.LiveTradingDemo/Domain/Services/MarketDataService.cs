...
{
    private readonly Dictionary<string, IObservable<MarketData>> _prices = 
        new Dictionary<string, IObservable<MarketData>>();
    ...
    private IObservable<MarketData> GenerateStream(CurrencyPair currencyPair)
    {
        ...
        return Observable.Create<MarketData>(observer =>
        {
            ...
            return Observable.Interval(TimeSpan.FromSeconds(1 / (double)currencyPair.TickFrequency))
                .Select(_ => random.Next(1, 5)) // selecting random pips number between 1 and 5
                .Subscribe(pips =>
                {
                    //move up or down between 1 and 5 pips
                    var adjustment = 
                        Math.Round(pips * currencyPair.PipSize, currencyPair.DecimalPlaces);
